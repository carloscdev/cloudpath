<template>
  <div
    class="group relative flex flex-col rounded-2xl border overflow-hidden"
    :class="[
      course.available
  ...
  >
    ...
    <div
      class="h-0.5 w-full"
      :class="course.available ? 'opacity-0 group-hover:opacity-100' : 'opacity-0'"
    ...
    <div class="p-6 flex flex-col flex-1">
      ...
      <div class="flex items-start justify-between mb-5">
        <!-- Provider icon -->
        <div
          class="w-12 h-12 rounded-xl flex items-center justify-center text-xl font-bold group-hover:scale-110"
        ...
        >
          <span v-if="course.providerLogo === 'aws'" class="text-base font-black tracking-tighter" :style="{ color: course.color }">AWS</span>
          <span v-else-if="course.providerLogo === 'azure'" class="text-xs font-black" :style="{ color: course.color }">Az</span>
      ...
      <div class="mb-3">
        <span
          class="inline-flex items-center px-2 py-0.5 text-xs font-mono font-medium rounded-md"
      ...
      <div class="flex items-center gap-4 mb-5 text-xs text-neutral-400 dark:text-neutral-500">
        ...
        <div class="flex items-center gap-1.5">
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            <circle cx="6" cy="6" r="5" stroke="currentColor" stroke-width="1.2"/>
            <path d="M4.5 5C4.5 4.17 5.17 3.5 6 3.5C6.83 3.5 7.5 4.17 7.5 5C7.5 5.83 6 6.5 6 7M6 8.5V9" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
          </svg>
      ...
      <div class="flex items-center justify-between border-t border-neutral-100 dark:border-neutral-800 pt-4">
        <div class="flex items-center gap-1.5 text-xs text-neutral-400 dark:text-neutral-500">
          ...
          <span class="font-medium text-neutral-600 dark:text-neutral-300">{{ course.level[currentLang] }}</span>
        </div>
        ...
        >
          ...
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" style="transition: transform 300ms cubic-bezier(0.34, 1.56, 0.64, 1);" class="group-hover/btn:translate-x-1">
            <path d="M2.5 6H9.5M9.5 6L6.5 3M9.5 6L6.5 9" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </button>
        ...
        <div v-else class="flex items-center gap-1.5 text-xs text-neutral-400 dark:text-neutral-500">
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M4 5.5V4C4 2.9 4.9 2 6 2C7.1 2 8 2.9 8 4V5.5" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/>
          </svg>
          {{ t.courses.comingSoon }}
        </div>
      </div>
